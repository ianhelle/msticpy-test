...
TEST_CASES = {
    'ipv4': r'c:\one\path\or\another\myprocess -ip4:206.123.1.123',
    'ipv6': r'''c:\one\path\or\another\myprocess -ip6:(2001:0db8:85a3:0000:0000:8a2e:0370:7334,
    2001:db8:85a3:0:0:8a2e:370:7334,2001:db8:85a3::8a2e:370:7334,::ffff:192.0.2.128)''',
    'url': r'''c:\one\path\or\another\myprocess /url:https://some.domain.it/thepath?qry1=abc&qry2=xyz
        /url:https://myuser@some.domain.es:88/thepath?qry1=abc&qry2=xyz''',
    'windows_path': r'c:\one\path\or\another\myprocess -file:"..\another\file" -file:"\\uncpath\file"',
    'linux_path': r'/bin/bash --file:./bish --file:/bin/bash --file:../../bosh',
    'md5_hash': '00236a2ae558018ed13b5222ef1bd987hash -something-hash=00236a2ae558018ed13b5222ef1bd987hash -something -hash=00236a2ae558018ed13b5222ef1bd987',
    'sha1_hash': '00236a2ae558018ed13b5222ef1bd98700000001hash -something -hash=00236a2ae558018ed13b5222ef1bd98700000001hash -something -hash=00236a2ae558018ed13b5222ef1bd98700000001',
    'sha256_hash': '''00236a2ae558018ed13b5222ef1bd98700000001123456789012345678901234hash -something -hash=00236a2ae558018ed13b5222ef1bd98700000001123456789012345678901234hash -something
...
class TestIoCExtractor(unittest.TestCase):
    ...
    def __run_extract(self, extractor=None, testcase=None, expected_items=None):
        ...
        test_input = TEST_CASES[testcase]
        results = extractor.extract(test_input)
        for k, v in expected_items.items():
            self.assertEqual(len(results[k]), v)
    ...
    def test_url(self):
        self.__run_extract(self.extractor, 'url', {'url': 2, 'dns': 2})
    ...
    def test_dataframe(self):
        ...
        self.assertEqual(output_df[output_df['IoCType'] == 'ipv4'].shape[0], 2)
        self.assertEqual(output_df[output_df['IoCType'] == 'ipv6'].shape[0], 2)
        self.assertEqual(output_df[output_df['IoCType'] == 'url'].shape[0], 2)
        self.assertEqual(output_df[output_df['IoCType'] == 'windows_path'].shape[0], 6)
        self.assertEqual(output_df[output_df['IoCType'] == 'linux_path'].shape[0], 7)
